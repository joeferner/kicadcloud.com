...
EdaItem.renderZipFile = function(edaItems, name, out, next) {
  return temp.mkdir('edaItems', function(err, dirPath) {
    ...
    return fs.mkdir(subDirPath, function(err) {
      if (err) {
        return next(err);
      }

      return EdaItem.renderModFile(edaItems, path.resolve(subDirPath, name + '.mod'), function(err) {
        if (err) {
          return next(err);
        }

        return EdaItem.renderLibFile(edaItems, path.resolve(subDirPath, name + '.lib'), function(err) {
          if (err) {
            return next(err);
          }
    ...
    function zipDirectory() {
      ...
      console.log('creating zip file:', subDirPath, zipFile);
...
EdaItem.renderModFile = function(edaItems, filename, callback) {
  var err = null;
  ...
  out.on('close', function() {
    console.log('done writing mod file:', filename);
    return callback(err);
  });
  ...
  async.forEach(edaItems, function(edaItem, callback) {
    ...
    if (edaItem.codeWrl && mod.shape3d && mod.shape3d.fileName) {
...
EdaItem.renderLibFile = function(edaItems, filename, callback) {
  ...
  out.on('close', function() {
    console.log('done writing lib file:', filename);
    return callback(err);
  });
