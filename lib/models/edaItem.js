...
EdaItem.findStats = function(conn, callback) {
  return async.auto({
    schematicSymbolCount: function(callback) {
      EdaItem
        .where('type = ?', EdaItem.types.schematicSymbol)
        .count(conn, callback);
    },
    pcbModuleCount: function(callback) {
      EdaItem
        .where('type = ?', EdaItem.types.pcbModule)
        .count(conn, callback);
    }
  }, callback);
};
...
EdaItem.search = function(conn, opts, callback) {
  ...
  opts.page = parseInt(opts.page || 0);

  return search.query(opts.q, function(err, searchResults) {
    if (err) {
      return callback(err);
    }

    var startIdx = opts.page * opts.count;
    searchResults = searchResults.slice(startIdx, startIdx + opts.count);
    ...
    return EdaItem
      ...
      .include('createdBy')
      .include('units')
      .all(conn, orderSearchResults);

    function orderSearchResults(err, edaItems) {
      if (err) {
        return callback(err);
      }
      edaItems.forEach(function(edaItem) {
        var searchResultMatches = searchResults.filter(function(sr) { return sr.id == edaItem.id; });
        if (searchResultMatches.length > 0) {
          edaItem.score = searchResultMatches[0].score;
        }
      });
      edaItems = edaItems.sort(function(a, b) {
        if (a.score = b.score) {
          return 0;
        }
        return a.score > b.score ? 1 : -1;
      });
      return callback(null, edaItems);
    }
  });
};
