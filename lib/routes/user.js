...
module.exports = function(app) {
  ...
  app.get('/user/:id/favorites.mod', app.withConnection(getUserFavoritesMod));
  app.get('/user/:id/favorites.lib', app.withConnection(getUserFavoritesLib));
  ...
  function getUserFavoritesMod(req, res, next) {
    var userId = req.params.id;
    ...
    models.UserFavoriteEdaItem.findByUserId(req.conn, userId, function(err, favorites) {
      ...
      return models.EdaItem.renderModFile(favorites, 'favorites.mod', res, next);
    });
  }

  function getUserFavoritesLib(req, res, next) {
    var userId = req.params.id;
    ...
    models.UserFavoriteEdaItem.findByUserId(req.conn, userId, function(err, favorites) {
      ...
      return models.EdaItem.renderLibFile(favorites, 'favorites.lib', res, next);
    });
  }
  ...
  function getUserFavorites(req, res, next) {
    ...
    models.UserFavoriteEdaItem.findByUserId(req.conn, userId, function(err, favorites) {
      ...
      return res.render('edaItem/list.ejs', {
        ...
        modFileDownloadUrl: '/user/' + userId + '/favorites.mod',
        libFileDownloadUrl: '/user/' + userId + '/favorites.lib'
      });
    });
  }
  ...
  function getUserItems(req, res, next) {
    ...
    models.EdaItem.findByUserId(req.conn, userId, function(err, items) {
      ...
      return res.render('edaItem/list.ejs', {
        ...
        items: items,
        showDownloadLinks: true,
        modFileDownloadUrl: '/user/' + userId + '/items.mod',
        libFileDownloadUrl: '/user/' + userId + '/items.lib'
      });
    });
  }
};
