...
module.exports = function(app) {
  ...
  function sendForgotPasswordEmail(user, callback) {
    ...
    var myMsg = new email.Email(emailData);
  ...
  function sendVerificationEmail(user, callback) {
    ...
    log.debug('emailData', emailData);
    var myMsg = new email.Email(emailData);
    log.debug('myMsg', myMsg.cmd);
    return myMsg.send(callback);
  }

  function sendNewUserNotification(user, callback) {
    callback = callback || function() {};
    log.info('sending new user notification email to: ' + user.username);
    var emailData = {
      from: "support@kicadcloud.com",
      to: "joe.m.ferner@gmail.com",
      subject: "KiCadCloud: New User Email",
