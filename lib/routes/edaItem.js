...
module.exports = function(app) {
  app.get('/schematicSymbol/all', app.withConnection(getAll));
  ...
  app.get('/pcbModule/all', app.withConnection(getAll));
  ...
  app.get('/edaItem/stats', app.withConnection(getStats));
  app.get('/edaItem/all.json', app.withConnection(getAllJson));
  app.get('/edaItem/all', app.withConnection(getAll));
  app.get('/edaItem/new', getEdaItemNew);
  app.get('/edaItem/:id/download', app.withConnection(getDownload));
  app.get('/edaItem/:id.json', app.withConnection(getJsonById));
  ...
  function getStats(req, res, next) {
    var edaItemStats = cache.get('edaItemStats');
    if (edaItemStats) {
      return res.json(edaItemStats);
    }
    return models.EdaItem.findStats(req.conn, function(err, stats) {
      if (err) {
        return next(err);
      }
      cache.set('edaItemStats', stats);
  ...
  function getJsonById(req, res, next) {
    ...
    return models.EdaItem.findById(req.conn, req.session.user, id, function(err, edaItem) {
      ...
      edaItem = models.EdaItem.toPublic(edaItem);
  ...
  function getAllJson(req, res, next) {
    var opts = {
      page: req.query.page || 0,
      count: 10
    };

    return models.EdaItem.findAll(req.conn, opts, function(err, edaItems) {
      if (err) {
        return next(err);
      }
      edaItems = models.EdaItem.toPublic(edaItems);
      return res.json(edaItems);
    });
  }
  ...
  function getAll(req, res, next) {
    ...
    if (req.url.indexOf('schematicSymbol') >= 0) {
      opts.type = models.EdaItem.types.schematicSymbol;
    } else if (req.url.indexOf('pcbModule') >= 0) {
      opts.type = models.EdaItem.types.pcbModule;
    }
  ...
  function postSave(req, res, next) {
    ...
    req.assert('code', 'Part code is too large. KiCadCloud is currently working to increase the limit.').len(0, 60000);
