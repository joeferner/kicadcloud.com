...
module.exports = function(opts) {
  log.email = 'joe.m.ferner@gmail.com';
  models.User.passwordSalt = opts.passwordSalt;
  ...
  loadPartials(function(err) {
    ...
    app.use(function(req, res, next) {
      ...
      res.locals.urlHelper = require('./url');
...
function requestBadUrls(req, res, next) {
  if (req.url.indexOf('baidu.com') >= 0
    || req.url.indexOf('google.com') >= 0
    || req.url.indexOf('soso.com') >= 0
    || req.url.indexOf('24x7-allrequestsallowed.com') >= 0) {
...
function handleErrors(err, req, res, next) {
  if (err) {
    log.error('url ' + req.url + ' had an error', err.stack || err);
